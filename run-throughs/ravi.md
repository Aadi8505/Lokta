# Run-Through: Ravi Kumar

**Profile**: 42, Kirana Store Owner, Mysuru  
**Goal**: ₹15,00,000 for shop expansion (inventory stock line + commercial delivery vehicle)  

---

## 1. Questions Asked & Answers Given

### Must Questions (Tier 1)
| # | Question | Answer Given | Engine Implication |
|---|---|---|---|
| 1 | What do you need the loan for? | **Business expansion / stock** | Triggers Business Loan evaluation |
| 2 | How much loan amount are you looking for? | **₹15,00,000** | Sets requested principal |
| 3 | What is your employment type? | **Self-employed** | Prompts for ITR documentation |
| 4 | What is your monthly cash/net business income? | **₹60,000** (range ₹40k – ₹80k) | Midpoint used for actual income; low end for stress |
| 5 | What is your total monthly existing EMI outflow? | **₹0** | No existing debt obligations |
| 6 | What are your monthly household expenses? | **₹20,000** | Modest living expenses |
| 7 | What is your age? | **42** | 23 years left till retirement age 65 |
| 8 | What is your credit score? | **Don't know / Unknown** | Maps to 'unknown' band — widens rate band without 300 penalty |

### Additional Questions (Tier 2)
| # | Question | Answer Given | Engine Implication |
|---|---|---|---|
| 9 | Have you filed an Income Tax Return (ITR)? | **Yes** | Uses audited ITR figure rather than discounting cash |
| 10 | What is your annual ITR income? | **₹4,20,000** | Lenders consider ₹35,000/month as verified base |
| 11 | Do you have collateral (property/gold)? | **Yes (Property)** | **Crucial inflection point: enables LAP comparison** |
| 12 | What is the estimated market value of the collateral? | **₹45,00,000** | 60% LTV on LAP = up to ₹27,00,000 capacity |
| 13 | Can a family member co-apply? | **Yes (Wife earns ₹18,000/mo)** | Boosts lender eligibility |
| 14 | Will this loan generate new monthly business income? | **Yes (₹15,00,000 stock/vehicle)** | Productive loan offset: adds conservative +₹7,500/mo cash buffer |
| 15 | How many months of expenses do you have in savings? | **4 months** | Adequate savings buffer |

---

## 2. Four Outputs Produced

### O1: Verdict
- **Decision**: **`Borrow` (with recommendation to compare LAP vs. Unsecured)**
- **Reason**: *"An unsecured loan of ₹15,00,000 would stretch your monthly obligations too high (over 90% FOIR). However, using your shop premises collateral for a secured loan / LAP brings the EMI into a comfortable range (25%–35% FOIR)."*
- **Supporting Reasons**:
  - The business expansion is productive, generating expected incremental income.
  - ₹45L unencumbered property provides prime security, enabling long tenure and low interest.

### O2: Maximum Amount
- **Product A (Unsecured Business Loan)**:
  - Max Lender Sanction: ₹6,50,000 – ₹8,00,000 (constrained by ₹35k ITR income)
  - Requested ₹15L is **NOT viable** under unsecured credit alone.
- **Product B (Loan Against Property - LAP)**:
  - Max Lender Sanction: ₹22,00,000 – ₹27,00,000 (backed by 60% LTV of ₹45L property)
  - Borrower Safe Carry: **₹18,00,000 – ₹24,00,000**
- **Recommendation**: Use **LAP / Secured Business Financing** for the ₹15L loan.

### O3: Fair Interest Rate & True APR
- **Unsecured Business Loan**: 16.0% – 28.0% (Wide unknown score spread, high cost)
- **LAP / Secured Business Loan**: **9.50% – 12.00%** (Nominal rate)
- **True APR (IRR)**: **10.25% – 13.10%** (incorporating 1.0% processing fee amortized over 7–10 years)
- **Insight**: Pledging shop collateral saves Ravi over ₹4,50,000 in interest over the loan life.

### O4: Safe EMI Ceiling & Stress Test
- **Safe EMI Ceiling**: **₹41,500 / month** (incorporates living costs + savings buffer + productive income credit)
- **Recommended Tenure**: **120 months (10 years) LAP** (Expected EMI: ₹19,800 – ₹21,500/mo)
- **Stress Test**:
  - **Income Drop (-20% shock)**: Stressed safe EMI remains ₹30,700/mo, easily servicing the ₹19.8k EMI.
  - **Rate Rise (+200 bps floating reset)**: EMI rises to ~₹21,600/mo, remaining well under the ₹41,500 safe ceiling.

---

## 3. Negotiation Card

```
┌─────────────────────────────────────────────────────────────────┐
│                    LOKTA NEGOTIATION CARD                       │
│  Borrower: Ravi Kumar (Self-Employed · Property Collateral)    │
├─────────────────────────────────────────────────────────────────┤
│  VERDICT: BORROW VIA SECURED LOAN (LAP)                         │
│  DO NOT ACCEPT AN UNSECURED BUSINESS LOAN AT 18%+               │
│  REQUESTED: ₹15,00,000   |   COLLATERAL PLEDGED: ₹45,00,000    │
├─────────────────────────────────────────────────────────────────┤
│  FAIR SECURED RATE: 9.50% – 12.00% (True APR: 10.25% – 13.10%) │
│  TARGET EMI: ₹19,800 – ₹21,500 / month (120 months)             │
│  SAFE EMI CEILING: Do not agree to an EMI above ₹41,500         │
├─────────────────────────────────────────────────────────────────┤
│  NEGOTIATION SCRIPTS FOR THE BRANCH:                            │
│  1. "I have ₹45L unencumbered property collateral; I will not   │
│     take an unsecured business loan at 18-24%. Quote me LAP."   │
│  2. "My wife is joining as co-applicant with ₹18,000/mo income  │
│     to satisfy debt-service coverage ratio."                    │
│  3. "I want a 7-to-10 year tenure to keep monthly EMI under     │
│     ₹22,000 while my new stock line ramps up."                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## 4. UI Screenshots

### 1. Results Dashboard — Executive KPI Strip & Assessment Verdict
Executive dashboard showing Ravi's assessment verdict under property collateral consideration, Safe Carry ceiling (₹18.9L – ₹30.7L), and safe EMI ceiling (₹34,000/mo).

![Ravi Assessment Verdict](./assets/ravi-assessment-verdict.png)

---

### 2. Borrowing Capacity, Product Comparison & Fair Rate Band
Detailed breakdown contrasting Unsecured Business Loan vs. Loan Against Property (LAP) and Secured Business Loan, quoting 10.5% – 16.5% base rate for LAP.

![Ravi Product Comparison](./assets/ravi-product-comparison.png)

---

### 3. Branch Ready Dossier — Borrower Negotiation Card
One-page printable negotiation card summarizing Ravi's borrowing limits, critical questions to ask the loan officer before signing, and branch negotiation rules.

![Ravi Negotiation Card](./assets/ravi-negotiation-card.png)

